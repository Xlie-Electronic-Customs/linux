...
#if IS_ENABLED(CONFIG_MTD_SPINAND_ECONET)
...
static int spinand_block_markbad(struct nand_chip *chip, loff_t offset)
{
	_SPI_NAND_DEBUG_PRINTF(SPI_NAND_FLASH_DEBUG_LEVEL_1, "[spinand_block_markbad]: enter , offset=0x%x\n", offset);
	return 0;
}

static int spinand_block_bad(struct nand_chip *chip, loff_t ofs)
{
...
static int spinand_dev_ready(struct nand_chip *chip)
{
	_SPI_NAND_DEBUG_PRINTF(SPI_NAND_FLASH_DEBUG_LEVEL_1, "[spinand_dev_ready]: enter \n");
	return 1;
}

static void spinand_enable_hwecc(struct nand_chip *chip, int mode)
{
	_SPI_NAND_DEBUG_PRINTF(SPI_NAND_FLASH_DEBUG_LEVEL_1, "[spinand_enable_hwecc]: enter \n");
}

static int spinand_correct_data(struct nand_chip *chip, u_char *dat,
...
static int spinand_calculate_ecc(struct nand_chip *chip, const u_char *dat, u_char *ecc_code)
{
...
static int en75xx_spinand_probe(void)
{
	...
	if (nand_scan_with_ids(chip, 1, spi_nand_flash_ids)) {
		pr_warn("%s fail!\n", "nand_scan_with_ids");
		err = -ENXIO;
		goto out_err_free;
	}
