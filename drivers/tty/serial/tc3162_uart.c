...
static void tc3162_uart_send_xchar(struct uart_port *port, char ch)
{
...
static inline int tc3162_uart_tx_ready(struct uart_port *port)
{
	return 1;
}
...
static void tc3162_uart_start_tx(struct uart_port *port)
{
	...
	while (tc3162_uart_tx_ready(port)) {
		copied = kfifo_out(&tport->xmit_fifo, buf, sizeof(buf));
		if (!copied)
			break;

		copied -= tc3162_uart_tx_buf_write(buf, copied);
		port->icount.tx += copied;
