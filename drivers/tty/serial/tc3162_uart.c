...
#include <linux/platform_device.h>	/* kernel 6.4+: serial_base needs a real device */
...
/*
 * kernel 6.4+ (serial_base): uart_add_one_port() walks port->dev->fwnode
 * unconditionally inside serial_base_ctrl_add(). We must provide a real
 * struct device, so we synthesise a platform_device at init time.
 */
static struct platform_device *tc3162_uart_pdev;
...
/* -------------------------------------------------------------------------
 * Internal software TX ring buffer
 * ---------------------------------------------------------------------- */
 ...
 * Returns the number of bytes that could NOT be written (leftover).
 * bytes_written = count - return_value.
 */
					     ...
					     unsigned int count)
{
	...
	return count; /* leftover = bytes NOT written */
}
...
/*
 * tc3162_uart_start_tx — called with interrupts disabled and port->lock held.
 *
 * Kernel 6.x removed struct circ_buf / uart_circ_chars_pending() from
 * uart_state. The TX data is now stored in a struct kfifo (xmit_fifo)
 * inside struct tty_port.
 *
 * kfifo_out_linear_ptr() returns a pointer to the next contiguous segment
 * and its byte count — exactly matching the old two-branch circ_buf logic
 * (wrap segment + remainder), handled across two loop iterations.
 *
 * tc3162_uart_tx_buf_write() returns leftover bytes (NOT bytes written), so:
...
static void tc3162_uart_start_tx(struct uart_port *port)
{
	...
	unsigned int count, remaining, written;
	unsigned char *buf;
	...
	/*
	 * Loop handles the circular-buffer wrap transparently: first iteration
	 * covers data up to the physical end of the kfifo buffer, second
	 * iteration (if any) covers the wrapped portion from the start.
	 */
	while ((count = kfifo_out_linear_ptr(&tport->xmit_fifo,
					     &buf, UART_XMIT_SIZE)) > 0) {
		remaining = tc3162_uart_tx_buf_write(buf, count);
		written   = count - remaining;

		port->icount.tx += written;
		kfifo_skip_count(&tport->xmit_fifo, written);

		/*
		 * Internal ring buffer is full — stop pushing.
		 * The TX thread will call uart_write_wakeup() after draining,
		 * which re-triggers start_tx via the tty layer.
		 */
		if (remaining > 0)
			break;
	}
...
/*
 * tc3162_uart_set_termios — kernel 6.x changed the third argument from
 * "struct ktermios *old" to "const struct ktermios *old".
 */
				      ...
				      unsigned int count)
{
	...
	while (count > 0) {
		...
		/* tc3162_uart_tx_thread_flushing flag alone is not sufficient
		 * since the TX thread may lose CPU during flushing.
		 */
		if (!atomic_read(&tc3162_uart_tx_thread_flushing) ||
		    cpu == task_thread_info(tc3162_uart_tx_thread)->cpu) {
			/* the buffer cannot be flushed by the TX thread */
...
static struct uart_port tc3162_uart_ports[TC3162_UART_NR_PORTS] = {
	{
		...
		/*
		 * .dev is intentionally left NULL here; it is assigned in
		 * tc3162_uart_init() after the platform_device is created,
		 * before uart_add_one_port() is called.
		 */
	}
};
...
static struct uart_driver tc3162_uart_driver = {
	...
	.cons		= &tc3162_uart_console,	/* wire console to driver */
};
...
static int __init tc3162_uart_init(void)
{
	...
	/*
	 * Kernel 6.4 introduced the serial_base layer which is called from
	 * uart_add_one_port() → serial_core_register_port() →
	 * serial_base_ctrl_add() → __dev_fwnode(port->dev).
	 *
	 * port->dev MUST point to a valid struct device before the call or
	 * the kernel will fault at the fwnode offset (~0xc4) of a NULL ptr.
	 *
	 * This driver has no DT/ACPI description and no platform_driver
	 * probe path, so we synthesise a minimal platform_device to satisfy
	 * the requirement.
	 */
	tc3162_uart_pdev = platform_device_alloc("tc3162-uart", 0);
	if (!tc3162_uart_pdev) {
		ret = -ENOMEM;
		goto unreg_driver;
	}

	ret = platform_device_add(tc3162_uart_pdev);
	if (ret) {
		pr_err("unable to add platform device\n");
		goto put_pdev;
	}

	/* Wire the port to the real device BEFORE registering. */
	tc3162_uart_ports[0].dev = &tc3162_uart_pdev->dev;
...
del_pdev:
	tc3162_uart_ports[0].dev = NULL;
	platform_device_del(tc3162_uart_pdev);
put_pdev:
	platform_device_put(tc3162_uart_pdev);
	tc3162_uart_pdev = NULL;
unreg_driver:
...
static void __exit tc3162_uart_exit(void)
{
	unsigned long flags;

	uart_remove_one_port(&tc3162_uart_driver, &tc3162_uart_ports[0]);
	tc3162_uart_ports[0].dev = NULL;

	platform_device_unregister(tc3162_uart_pdev);
	tc3162_uart_pdev = NULL;
