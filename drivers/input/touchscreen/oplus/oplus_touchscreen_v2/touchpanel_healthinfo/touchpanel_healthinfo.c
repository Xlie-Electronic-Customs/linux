...
static bool is_point_reporting(int obj_attention, struct point_info *points, int num)
{
	return (((obj_attention & TOUCH_BIT_CHECK) >> num) & 0x01
		&& (points[num].status != 0));
}

static bool is_point_in_zone(struct list_head *zone_list, struct point_info cur_p,
		      int direction)
{
...
static int add_point_to_record(struct points_record *points_record,
...
static int add_swipe_to_record(struct swipes_record *swipes_record,
...
static int print_point_from_record(struct seq_file *s,
...
static int print_swipe_from_record(struct seq_file *s,
...
static int update_value_count_list(struct list_head *list, void *value,
...
static int clear_value_count_list(struct list_head *list)
{
	struct list_head *pos = NULL;
	struct health_value_count *vc = NULL;
...
static int print_value_count_list(struct seq_file *s, struct list_head *list,
...
static char *print_as_matrix(struct seq_file *s, void *value, int len, int linebreak,
		      bool need_feedback)
{
...
static char *record_buffer_data(struct list_head *list, char *record,
...
static void print_buffer_list(struct seq_file *s, struct list_head *list, char *prefix)
{
	struct list_head *pos = NULL;
	struct health_value_count *vc = NULL;
...
static void print_delta_data(struct seq_file *s, int32_t *delta_data, int tx_num,
		      int rx_num)
{
	char *tmp_str = NULL;
	char *tmp_str_child = NULL;
...
static void clear_delta_data(int32_t *delta_data, int TX_NUM, int RX_NUM)
{
	int i = 0, j = 0;
...
static bool is_delta_data_allzero(int32_t *delta_data, int tx_num, int rx_num)
{
	int i = 0;
...
static int catch_delta_data(struct monitor_data *monitor_data, int32_t *delta_data)
{
...
static int point_state_up_handle(struct monitor_data *monitor_data, int i,
			  int direction)
{
...
static int tp_touch_healthinfo_handle(struct monitor_data *monitor_data,
...
static int tp_raw_touch_healthinfo_handle(struct monitor_data *monitor_data,
...
static int tp_grip_up_healthinfo_handle(struct monitor_data *monitor_data, uint8_t up_id,
				 int direction)
{
	int ret = 0;
...
static int tp_gesture_healthinfo_handle(struct monitor_data *monitor_data,
				 int gesture_type)
{
	if (!monitor_data) {
		return 0;
...
static int tp_gesture_read_healthinfo_handle(struct monitor_data *monitor_data,
				 int gesture_type)
{
	if (!monitor_data) {
		return 0;
...
static int tp_fingerprint_healthinfo_handle(struct monitor_data *monitor_data,
				     uint8_t area_rate)
{
	int rate_rate = 0;
...
static int tp_face_detect_healthinfo_handle(struct monitor_data *monitor_data,
				     int ps_state)
{
	if (!ps_state) {
		return 0;
...
static int tp_report_healthinfo_handle(struct monitor_data *monitor_data, char *report)
{
	int ret = 0;
...
static int tp_irq_interval_handle(struct monitor_data *monitor_data, int count)
{
    if (!monitor_data) {
        return 0;
    }
...
static int tp_grip_healthinfo_handle(struct monitor_data *monitor_data,
...
static int tp_probe_healthinfo_handle(struct monitor_data *monitor_data,
			       u64 start_time)
{
	if (!monitor_data) {
		return 0;
...
static int update_max_time(struct monitor_data *monitor_data, u64 *max_time,
		    u64 start_time)
{
	u64 time_cost = 0;
...
static int tp_resume_healthinfo_handle(struct monitor_data *monitor_data,
				u64 start_time)
{
	if (!monitor_data) {
		return 0;
...
static int tp_suspend_healthinfo_handle(struct monitor_data *monitor_data,
				 u64 start_time)
{
	u64 screenon_time = 0;
...
static int tp_test_healthinfo_handle(struct monitor_data *monitor_data,
...
static int tp_bus_err_healthinfo_handle(struct monitor_data *monitor_data, int errno,
...
static int tp_alloc_healthinfo_handle(struct monitor_data *monitor_data,
...
static int tp_fw_update_healthinfo_handle(struct monitor_data *monitor_data,
				   char *result)
{
	int ret = 0;
	char *report = NULL;
...
static int tp_voltage_healthinfo_handle(struct monitor_data *monitor_data,
...
static int tp_reclining_healthinfo_handle(struct monitor_data *monitor_data,
		struct kernel_grip_info *grip_info)
{
	u64 time_cost = 0;
...
static int init_grip_zone(struct device *dev, struct monitor_data *monitor_data)
{
	int ret = 0;
