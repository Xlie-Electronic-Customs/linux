...
// #include "../../../../../pinctrl/qcom/pinctrl-msm.c"
#include "../touchpanel_common.h"
...
struct msm_pinctrl {
	...
	DECLARE_BITMAP(dual_edge_irqs, 300);
	DECLARE_BITMAP(enabled_irqs, 300);
	DECLARE_BITMAP(skip_wake_irqs, 300);
	DECLARE_BITMAP(disabled_for_mux, 300);
...
bool msm_gpio_get_pin_address(unsigned int gpio_num, struct resource *res)
{
	...
	unsigned int reg_size_4k = 1; /* default: 1 * 4K */

	res->end = res->start +
	max(reg_size_4k, 1U) * SZ_4K - 1;
	// res->end = res->start + (g->reg_size_4k ? : 1) * SZ_4K - 1;
...
static void touchpanel_trusted_touch_tvm_vm_mode_enable(struct touchpanel_data *ts)
{
	...
	if (ts->bus_type == TP_BUS_I2C)
		...
		rc = pm_runtime_get_sync(ts->s_client->controller->dev.parent);
...
static void touchpanel_trusted_touch_tvm_vm_mode_disable(struct touchpanel_data *ts)
{
	...
	if (ts->bus_type == TP_BUS_I2C)
		...
		pm_runtime_put_sync(ts->s_client->controller->dev.parent);
...
static int touchpanel_bus_get(struct touchpanel_data *ts)
{
	...
	if (ts->bus_type == TP_BUS_I2C) {
		...
		dev = ts->s_client->controller->dev.parent;
	}
