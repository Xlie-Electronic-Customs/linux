					...
					const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (count > PALM_BUF_MAX) {
		TPD_INFO("%s over size cmd is %lu\n", __func__, count);
		goto OUT;
	}
...
static ssize_t proc_gesture_control_indep_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (ts->sportify_aod_gesture_support &&
		ts->is_suspended &&
		!!ts->gesture_enable) {
		TPD_INFO("%s:[SPORITFY AOD enable]set %d gesture\n", __func__, value);
...
static ssize_t proc_pencil_connect_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (count > BUF_SIZE - 1) {
		TPD_INFO("%s over size control cmd is %lu\n", __func__, count);
		return count;
	}
				   ...
				   size_t count, loff_t *ppos)
{
	...
	if (!ts->ts_ops->force_water_mode) {
		TPD_INFO("%s: ts->ts_ops->force_water_mode is NULL\n", __func__);
		return count;
	}

	if (ts->is_suspended) {
		TPD_INFO("%s: is_suspended, exit\n", __func__);
		return count;
	}
 ...
 */
static int init_touchpanel_proc_part2(struct touchpanel_data *ts, struct proc_dir_entry *prEntry_tp)
{
	int ret = 0;
	int i = 0;
