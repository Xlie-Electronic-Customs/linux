...
static int haptics_pbs_trigger_isc_config(struct haptics_chip *chip)
{
	...
	pdev = of_find_device_by_node(chip->pbs_node);
	if (!pdev) {
		dev_err(chip->dev, "Unable to find PBS dev_node\n");
		return -ENODEV;
	}

	pbs = platform_get_drvdata(pdev);

	rc = qcom_pbs_trigger_single_event(pbs);
	if (rc < 0)
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	static ssize_t lra_frequency_hz_store(const struct class *c,
			const struct class_attribute *attr, const char *buf, size_t count)
	{
		bool val;
