...
static int haptics_parse_dt(struct haptics_chip *chip)
{
	...
	if (chip->trig_support) {
		chip->trig_gpios = devm_gpiod_get_array(dev, "oplus,trig", GPIOD_OUT_LOW);
		for (i = 0; i < TRIG_GPIO_NUM; i++) {
			if (!IS_ERR(chip->trig_gpios)) {
				gpiod_direction_input(chip->trig_gpios->desc[i]);
				chip->trig_irq[i] = gpiod_to_irq(chip->trig_gpios->desc[i]);
...
static int haptics_pbs_trigger_isc_config(struct haptics_chip *chip)
{
	...
	rc = qcom_pbs_trigger_single_event(chip->pbs_node);
	if (rc < 0)
...
static int haptics_probe(struct platform_device *pdev)
{
	...
	raw_notifier_chain_register(&hboost_notifier, &chip->hboost_nb);
...
static void haptics_remove(struct platform_device *pdev)
{
	...
	raw_notifier_chain_unregister(&hboost_notifier, &chip->hboost_nb);
...
static int __maybe_unused haptics_suspend(struct device *dev)
{
	...
	if(chip->trig_support) {
		if (device_may_wakeup(dev)) {
			for (i = 0; i < TRIG_GPIO_NUM; i++) {
				if (!IS_ERR(chip->trig_gpios) && chip->trig_irq[i] > 0)
