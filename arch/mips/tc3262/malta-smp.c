...
void __init vsmp_int_init(void)
{
	...
	ret = request_irq(MIPS_CPU_IPI_RESCHED_IRQ, ipi_resched_interrupt, IRQF_PERCPU, "ipi_resched", NULL);
	if (ret) {
		pr_err("%s: failed to register irq\n", __func__);
	}
	irq_set_handler(irq, handle_percpu_irq);

	ret = request_irq(MIPS_CPU_IPI_CALL_IRQ, ipi_call_interrupt, IRQF_PERCPU, "ipi_call", NULL);
	if (ret) {
		pr_err("%s: failed to register irq\n", __func__);
	}
	irq_set_handler(irq, handle_percpu_irq);
