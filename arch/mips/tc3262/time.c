...
void __init plat_time_init(void)
{
	...
	ret = request_irq(BUS_TOUT_INT, tc_bus_timeout_interrupt, IRQF_NO_THREAD, "bus timeout", NULL);
	if (ret) {
		pr_err("%s: failed to register irq\n", __func__);
	}

#ifdef CONFIG_ECONET_EN7528
	ret = request_irq(RBUS_TOUT_INTR, rbus_timeout_interrupt, IRQF_NO_THREAD, "rbus timeout", NULL);
	if (ret) {
		pr_err("%s: failed to register irq\n", __func__);
	}
