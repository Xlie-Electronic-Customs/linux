...
void __init arch_init_irq(void)
{
	...
	ret = request_irq(CPU_CM_ERR, cpu_cm_err_interrupt, IRQF_NO_THREAD, "cpu_cm_err", NULL);
	if (ret) {
		pr_err("%s: failed to register irq\n", __func__);
	}

	ret = request_irq(CPU_CM_PCINT, cpu_cm_pcint_interrupt, IRQF_NO_THREAD, "cpu_cm_pcint", NULL);
	if (ret) {
		pr_err("%s: failed to register irq\n", __func__);
	}
}
